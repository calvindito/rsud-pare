                    ...
                    <tbody>
                        @foreach($chartOfAccount as $coa)
                            @if($coa->sub->count() > 0)
                                ...
                                @include('finance.budget-recursive', ['sub' => $coa->sub, 'left' => 20, 'budget' => $budget, 'disabled' => true])
                            @else
                                <tr>
                                    <input type="hidden" name="bd_chart_of_account_id[]" value="{{ $coa->id }}">
                                    <td>{{ $coa->name }}</td>
                                    <td>
                                        <input type="text" class="form-custom number-format" name="bd_nominal[]" value="{{ $budget->budgetDetail()->firstWhere('chart_of_account_id', $coa->id)->nominal ?? '' }}" placeholder="0" disabled>
                                    </td>
                                    <td>
                                        <input type="text" class="form-custom number-format" name="bd_limit_blud[]"value="{{ $budget->budgetDetail()->firstWhere('chart_of_account_id', $coa->id)->limit_blud ?? '' }}" placeholder="0" disabled>
                                    </td>
                                </tr>
                            @endif
                        @endforeach
