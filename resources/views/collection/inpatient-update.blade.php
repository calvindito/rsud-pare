                    ...
                    <div class="col-md-9">
                        <select class="form-select" name="dispensary_id" id="dispensary_id">
                            ...
                            @foreach($dispensary as $d)
                                <option value="{{ $d->id }}" {{ $inpatient->dispensary_id == $d->id ? 'selected' : '' }}>
                                    {{ $d->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @if($action->count() > 0)
                            @foreach($action as $key => $a)
                                <tr>
                                    ...
                                    <td width="20%">
                                        <input type="number" class="form-control" name="ial_limit[]" value="{{ $inpatient->inpatientActionLimit()->firstWhere('action_id', $a->id)->limit ?? 2 }}">
                                    </td>
                                </tr>
                            @endforeach
                        @else
